...
import no.nr.einar.pb.analysis.code.dataflow.FieldFlowInterpreter;
import no.nr.einar.pb.analysis.code.descriptor.FieldDescriptorParser;
...
//		}
	...
	private void checkDegenerates(AbstractInsnNode[] instructions,
			MethodAnalysisData data) {
/*
		if (isEmptyMethodBody(instructions)) {
			data.setAttribute(Attribute.DEGENERATE_EMPTY);
		}
		
		if (isLazyGetter(instructions)) {
			data.setAttribute(Attribute.DEGENERATE_LAZY_GETTER);
		}
		
		if (alwaysThrowsException(instructions)) {
			data.setAttribute(Attribute.DEGENERATE_EXCEPTION);
		}
		
		if (alwaysReturnsNull(instructions)) {
			data.setAttribute(Attribute.DEGENERATE_NULL);
		}
		
		if (alwaysReturnsSmallIntConstant(instructions)) {
			if (alwaysReturnsBitConstant(instructions)) {
				data.setAttribute(Attribute.DEGENERATE_BIT_CONSTANT);
			} else {
				data.setAttribute(Attribute.DEGENERATE_INT_CONSTANT);
			}
		}
		...
		if (alwaysReturnsConstant(instructions)) {
			data.setAttribute(Attribute.DEGENERATE_CONSTANT);
		}
		
		if (alwaysReturnsFieldValue(instructions)) {
			data.setAttribute(Attribute.DEGENERATE_FIELD_READ);
		}
		
		final DelegationFinder delFinder = new DelegationFinder(isStatic);
		if (delFinder.delegates(instructions)) {
			data.setAttribute(Attribute.DEGENERATE_DELEGATION);
			++delegatingMethods;
		}
		*/
	}
	...
	private boolean isLazyGetter(final AbstractInsnNode[] instructions) {
		return isInstanceLazyGetter(instructions) || isStaticLazyGetter(instructions);
	}

	private void dumpInstructions(final AbstractInsnNode[] instructions) {
		for (int i = 0; i < instructions.length; i++) {
			System.out.println(Bytecode.bytecode(instructions[i].getOpcode()));
		}
	}

	private boolean isStaticLazyGetter(final AbstractInsnNode[] instructions) {
		if (instructions.length == 9) {
			final int[] opcodes = new int[] {
					Opcodes.GETSTATIC, Opcodes.IFNONNULL, 
					Opcodes.NEW, Opcodes.DUP, Opcodes.INVOKESPECIAL, Opcodes.PUTSTATIC,
					-1, Opcodes.GETSTATIC, Opcodes.ARETURN
			};
			for (int i = 0; i < opcodes.length; i++) {
				if (opcodes[i] != instructions[i].getOpcode()) {
					return false;
				}
			}
			return true;
	...
	private boolean isInstanceLazyGetter(final AbstractInsnNode[] instructions) {
		if (instructions.length == 12) {
			final int[] opcodes = new int[] {
					Opcodes.ALOAD, Opcodes.GETFIELD, Opcodes.IFNONNULL, 
					Opcodes.ALOAD, Opcodes.NEW, Opcodes.DUP, Opcodes.INVOKESPECIAL, Opcodes.PUTFIELD, 
					-1, Opcodes.ALOAD, Opcodes.GETFIELD, Opcodes.ARETURN
			};
			for (int i = 0; i < opcodes.length; i++) {
				if (opcodes[i] != instructions[i].getOpcode()) {
					return false;
	...
	private boolean alwaysReturnsFieldValue(final AbstractInsnNode[] instructions) {
		if (instructions.length == 3) {
			if (instructions[0].getOpcode() == Opcodes.ALOAD 
				&& instructions[1].getOpcode() == Opcodes.GETFIELD) {
				final VarInsnNode vin = (VarInsnNode) instructions[0];
				if (vin.var == 0) {
					final int opcode = instructions[2].getOpcode();
					final int[] opcodes = new int[] { Opcodes.ARETURN, Opcodes.IRETURN, 
							Opcodes.LRETURN, Opcodes.DRETURN, Opcodes.FRETURN };
					for (int i = 0; i < opcodes.length; i++) {
						if (opcode == opcodes[i]) {
							return true;
						}
	...
	private boolean alwaysReturnsBitConstant(AbstractInsnNode[] instructions) {
	...
	private void printInheritance(final ClassNode classNode) {
		System.err.println("Extends: " + classNode.superName);
		final Iterator itor = classNode.interfaces.iterator();
		while (itor.hasNext()) {
			final String s = (String) itor.next();
			System.err.println("Implements: " + s);
		}
	}

	private static boolean isRunMethod(final MethodNode node) {
		return "run".equals(node.name);
	}

	private static boolean isThreadingType(final ClassNode node) {
		return extendsThreadingClass(node) || implementsThreadingInterface(node);
	}

	private static boolean extendsThreadingClass(final ClassNode node) {
		return ("java/lang/Thread".equals(node.superName)) ||
			("java/util/TimerTask".equals(node.superName));
	}

	private static boolean implementsThreadingInterface(final ClassNode node) {
		final Iterator itor = node.interfaces.iterator();
		while (itor.hasNext()) {
			final String s = (String) itor.next();
			if ("java/lang/Runnable".equals(s)) {
				return true;
			}
		}
		return false;
