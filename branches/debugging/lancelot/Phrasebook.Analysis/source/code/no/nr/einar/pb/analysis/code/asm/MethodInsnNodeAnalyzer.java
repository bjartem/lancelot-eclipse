...
import no.nr.einar.pb.analysis.code.dataflow.TraceValue;
...
//						attr = Attribute.FAKE_RECURSION;
	...
	private boolean isThisUsedAsParameter(final Frame frame, final MethodAnalysisData data) {
		return isParameter(frame, data, new ValuePredicate() {
			@Override
			public boolean check(Value v) {
				if (v instanceof TraceValue) {
					return ((TraceValue) v).isMaybeThisReferenceValue(); 
				}
				return false;
			}
		});
